...
10. ~~`guardian-ui`: 13 `.spec.ts` ama hepsi iskelet~~ — ✅ **ÇÖZÜLDÜ (2026-07-13).** 11 değersiz "should create/be created" iskelet spec'i kaldırıldı; yerine 4 gerçek mantık testi eklendi: `time-utils.spec.ts` (formatRemaining/formatDuration/windowPercent), `status-labels.spec.ts` (statusLabel/statusTone eşlemeleri), `auth.service.spec.ts` (login/logout/hasRole rütbe/storage okuma), `auth-interceptor.spec.ts` (Bearer başlığı + 401'de logout, 403'te logout yok). Toplam 32 test geçiyor (`ng test`, headless).
