...
1.  **Sertifikaları Kopyalama:**
    Bu sunucu için gerekli olan TLS sertifikalarının oluşturulması ve dağıtımı, **[TLS Sertifikalarını Oluşturma ve Dağıtma](./generate-certs-usage.md)** dosyasında detaylı olarak anlatılmıştır. Lütfen o bölümdeki adımları takip ederek aşağıdaki dosyaları bu sunucudaki `/etc/guardian/certs/` dizinine kopyalayın:
    *   `ca.crt`
    *   `server.crt`
    *   `server.key`
